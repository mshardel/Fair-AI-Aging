...
# Sensitivity Analysis: HAC Recoded as Three-Level Categorical Variable
# Based on internal_crossvalidation_evaluation.R (random 70/30 split within 2010-2017, 50 replications)
#
# Instead of excluding subjects with missing HAC, HAC is recoded into three levels:
#   Missing (NA) -> HAC_new = 0
#   No  (HAC==0) -> HAC_new = 1  [reference level]
#   Yes (HAC==1) -> HAC_new = 2
#
# Two dummy variables replace the original binary HAC covariate:
#   HAC_yes     = 1 if HAC_new == 2 (Yes), 0 otherwise
#   HAC_missing = 1 if HAC_new == 0 (Missing), 0 otherwise
####################################################################################################
...
run_one_replication <- function(data, train_ratio = 0.7) {
  ...
  ## Train-Test Split: 70/30 random split within 2010-2017 data
  pool_data  <- data[data$yearAdmission != 8, ]
  n          <- nrow(pool_data)
  train_size <- floor(n * train_ratio)
  shuffled   <- sample(n)

  train_data <- pool_data[shuffled[1:train_size], ]
  test_data  <- pool_data[shuffled[(train_size + 1):n], ]

  ## Covariate matrices — HAC_yes and HAC_missing replace the original binary HAC
  covariates <- c("ALZDMT_PRIOR", "HAC_yes", "HAC_missing", "LOS_DAY_CNT", "PRE_DAYS",
...
main <- function() {
  ...
  ## Exclude only subjects with missing TERT_BED_SIZE; HAC missing is retained
  data <- data[!is.na(data$TERT_BED_SIZE), ]
  data <- data[data$AGECAT != 0, ]

  ## Recode HAC into three levels: Missing=0 (new), No=1 (ref), Yes=2
  data$HAC_new     <- ifelse(!is.na(data$HAC) & data$HAC == 1, 2L,
                       ifelse(!is.na(data$HAC) & data$HAC == 0, 1L, 0L))
  data$HAC_yes     <- ifelse(data$HAC_new == 2, 1, 0)   # Yes vs. reference (No)
  data$HAC_missing <- ifelse(data$HAC_new == 0, 1, 0)   # Missing vs. reference (No)

  ## Other derived variables (unchanged)
...
# --- Save results ---
write_xlsx(final_sheets, "output/revision/Analysis2_v5_HAC_results.xlsx")
